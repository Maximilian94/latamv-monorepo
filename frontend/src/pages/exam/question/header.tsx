...
export const QuestionHeader = ({ isEditing }: { isEditing: boolean }) => {
  const { currentQuestion, currentExamTemplate, updateQuestionTime } = useExamStore();
  const [isEditingTime, setIsEditingTime] = useState(false);
  const [time, setTime] = useState(currentExamTemplate?.timeLimit);

  const handleSaveTime = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();
    updateQuestionTime(Number(time || 0));
    setIsEditingTime(false);
  }

  const questionTimeFormatted = useMemo(() => {
    return convertMinutesTo_HH_MM(currentExamTemplate?.timeLimit || 0);
  }, [currentExamTemplate?.timeLimit]);

  return (
    <>
      <div className="flex flex-row justify-between gap-4 items-center py-2 px-4 text-lg text-slate-200">
        <span>
          Q {currentQuestion + 1}/{currentExamTemplate?.questions?.length}
        </span>
        <span>12357486</span>
